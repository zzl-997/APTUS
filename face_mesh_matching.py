...
if __name__ == '__main__':
    ...
    image_dict = {
        'left': cv2.imread('path/to/left_image.jpg'),
        'front': cv2.imread('path/to/front_image.jpg'),
        'right': cv2.imread('path/to/right_image.jpg')
    }
    
    with open('path/to/detection_results.json', 'r') as f:
        detection_results = json.load(f)
    
    # Detection results should have this structure:
    # {
    #     'left': {
    #         'det_boxes': {'class_name': [[x1,y1,x2,y2], ...]},
    #         'scores': {'class_name': [0.8, 0.6, ...]}
    #     },
    #     'front': {...},
    #     'right': {...}
    # }
    ...
    try:
        optimized_results = optimizer.process(detection_results, image_dict)
        for view in ['left', 'front', 'right']:
            view_result = optimized_results[view]
            print(f"\n=== {view.upper()} VIEW OPTIMIZED RESULTS ===")
            print(f"Number of detections: {len(view_result['boxes'])}")
            # Show matched points
            for i, (box, class_name, score, show, match_idx) in enumerate(zip(
                view_result['boxes'],
                view_result['classes'],
                view_result['scores'],
                view_result['show'],
                view_result['match_indices']
            )):
                if show:  # Only show high-confidence or matched points
                    match_status = f" (matched with point {match_idx})" if match_idx > 0 else ""
                    print(f"  {i}: {class_name} - score: {score:.3f} - box: {box}{match_status}")
